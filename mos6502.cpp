...
void mos6502::Run(uint32_t n, MainWindow *mainWindow)
{
    uint8_t opcode;
    Instr instr;
    ...
    while(n-- > 0 && !illegalOpcode)
    {
        ...
        char output[1024];
        static uint8_t mem[1024*64];
        for (uint16_t p=pc;p<pc+4;p++) mem[p] = Read(p);
        options_t options;
        options.org=pc;
        options.offset=0;
        options.hex_output=true;
        options.cycle_counting=true;
        uint16_t dummy_pc = pc;

        disassemble(output, mem, &options, &dummy_pc);
        ...
        //mainWindow->showInfo(output, this, dummy_pc-1, A, X, Y);
    }
}
